...
// Given an array of names, separate those starting with 'A' into one   list and the rest into another—do this in your favorite language (no specific one mentioned). This is language-independent but reveals if command over basics like loops, conditionals, and lists/arrays is solid enough for DSA.

const myArr = ["Ashish", "Biswas", "Dev", "Shivam", "Aditya", "Ashvin", "Tirth", "Dhrumal", "Karan"]

function filterNames(alphabetCharacter, arr) {
  // loop over the given arr --> find all the names started with the given alphabetCharacter --> add seperate them into different array

  const arrWithGivenAlphabet = []
  const arrWithoutGivenAlphabet = []

  arr.map((name) => {
    const index = 0
    if (name.at(index).toUpperCase() === alphabetCharacter) {
      arrWithGivenAlphabet.push(name)
    } else {
      arrWithoutGivenAlphabet.push(name)
    }
  })

  return { arrWithoutGivenAlphabet, arrWithGivenAlphabet }

}

console.log(filterNames("C", religiousCities));
